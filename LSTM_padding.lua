...
require 'nn'
require 'rnn'
local LSTM_padding, parent = torch.class('LSTM_padding', 'nn.AbstractRecurrent')

function LSTM_padding:__init(inputSize, outputSize, rho, cell2gate)
...
-------------------------- factory methods -----------------------------
function LSTM_padding:buildGate()
...
function LSTM_padding:buildInputGate()
   self.inputGate = self:buildGate()
   return self.inputGate
end

function LSTM_padding:buildForgetGate()
   self.forgetGate = self:buildGate()
   return self.forgetGate
end

function LSTM_padding:buildHidden()
...
function LSTM_padding:buildCell()
   -- build
...
function LSTM_padding:buildOutputGate()
...
function LSTM_padding:buildModel()
   -- build components
...
------------------------- forward backward -----------------------------
function LSTM_padding:updateOutput(input)
   --print(input)
   ...
   --[[
   for i=1, (#input)[1] do
      if #(input[i][(input[i]):eq(0)]):totable() == #(input[i]):totable() then
         output[i] = prevOutput[i] 
         cell[i] = prevCell[i]
      end
   end
   --]]
   --torch.repeatTensor(torch.reshape(a, 3, 1),3,1)
   --[[
   mask = torch.sum(input,2):gt(0)
   mask1 =torch.repeatTensor(mask, 1, (#input)[2])
   I = torch.ones(#mask1):cuda()
   mask2 = I - mask1
   output = torch.cmul(output, mask1) + torch.cmul(prevOutput, mask2)
   cell = torch.cmul(cell, mask1) + torch.cmul(prevCell, mask2)
   --]]
...
function LSTM_padding:_updateGradInput(input, gradOutput)
...
function LSTM_padding:_accGradParameters(input, gradOutput, scale)
